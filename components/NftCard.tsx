const NftCard=({name,tokenId,price,linkToDetails,imgUrl}:{imgUrl:string,name:string,tokenId:string,price:string,linkToDetails:string})=>{
    return(
      <div className='w-[220px'>
          <div className="border border-brandbrown rounded-lg ">
        <div className=' bg-[#e4e2e2] p-24 rounded-t-md w-full h-full' style={{backgroundImage:`url(${imgUrl})`,backgroundSize:'cover'}}></div> </div>
        <div className='border border-brandbrown p-4 rounded-b-md'>
      ...
      >
        ...
        ></path>
              ...
              <span className='ml-2'>{price}</span></div>
           <button className='border border-brandbrown py-1 px-6 rounded-md text-brandbrown font-semibold'>Details</button>
          </div>
        </div>
